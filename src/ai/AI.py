...
class AI:    
    ...
    def _initialize_client(self, ai_model):
        ...
        if not openai_api_key or not anthropic_api_key:
            raise AI_API_Key_Error("No API key was found - please head over to the troubleshooting notebook in this folder to identify & fix!")
        elif not openai_api_key.startswith("sk-proj-") or not anthropic_api_key.startswith("sk-ant-api"):
            raise AI_API_Key_Error("An API key was found, but it doesn't start sk-proj-; please check you're using the right key - see troubleshooting notebook")
        elif openai_api_key.strip() != openai_api_key or anthropic_api_key.strip() != anthropic_api_key:
            raise AI_API_Key_Error("An API key was found, but it looks like it might have space or tab characters at the start or end - please remove them - see troubleshooting notebook")
        elif not google_api_key.startswith("AIza"):
            raise AI_API_Key_Error("An API key was found, but it doesn't start AIza; please check you're using the right key - see troubleshooting notebook")
